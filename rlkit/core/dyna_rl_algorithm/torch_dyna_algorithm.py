...
class TorchDynaAlgorithm(BaseRLAlgorithm, metaclass=abc.ABCMeta):
    ...
    def _train_model(self, num_trains):
    ...
    def _train_agent(self, num_model_steps, num_policy_trains):
        total_steps = 0
        while total_steps < num_model_steps:
            model_steps = min(num_model_steps - total_steps,
            ...
            self.training_mode(True)
            for _ in range(num_policy_trains):
    ...
    def _end_epoch(self, epoch):
        super(TorchDynaAlgorithm, self)._end_epoch(epoch)
    ...
    def _get_snapshot(self):
        snapshot = super(TorchDynaAlgorithm, self)._get_snapshot()
